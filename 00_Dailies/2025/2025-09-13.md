(torch) sczd425@ln01:~/run/xinby/vcc_local$ python stage2_official_baseline_generation.py 
🎯 阶段2C: cell-eval官方baseline生成
============================================================
🎯 使用cell-eval官方方法生成baseline
============================================================
📂 加载训练数据: splits/train_fixed.h5ad
训练数据形状: (11326, 18080)
扰动条件: target_gene
non-targeting    7483
SDC1              170
HMGCR             166
NDUFB4            166
UQCRQ             166
DPH2              164
WAC               162
ATP6V0B           147
NISCH             144
METTL3            138
CHMP3             130
DNAJA3            127
RNF20             124
METTL17           122
METTL14           121
SSBP1             119
SMAGP             117
COX6C             117
SLC25A3           114
LRPPRC            110
STRAP             106
RRM1              103
HTATSF1            96
UQCRB              96
KIF20A             96
TFAM               82
SUPV3L1            80
SUPT4H1            74
MBTPS1             68
MAU2               66
HDAC3              60
ATP6V0C            53
PPP2R3C            52
MAT2A              52
EWSR1              48
EIF4B              41
SMARCE1            31
FDPS               10
SEC62               5
Name: count, dtype: int64
验证数据形状: (6115, 18080)

📊 验证集细胞数量分布:
  ARPC2: 200
  DHX36: 200
  C1QBP: 200
  JAZF1: 200
  HIRA: 200
  MED1: 200
  EIF3H: 200
  TMSB10: 200
  MED12: 200
  OXA1L: 200
  NDUFB6: 190
  USF2: 185
  SMARCB1: 182
  MAX: 180
  DNMT1: 170

📋 Baseline细胞数量配置:
   target_gene  n_cells
0        ARPC2      200
1        DHX36      200
2        C1QBP      200
3        JAZF1      200
4         HIRA      200
5         MED1      200
6        EIF3H      200
7       TMSB10      200
8        MED12      200
9        OXA1L      200
10      NDUFB6      190
11        USF2      185
12     SMARCB1      182
13         MAX      180
14       DNMT1      170

🔧 调用cell-eval官方baseline生成方法...
INFO:cell_eval._evaluator:Input is found to be log-normalized already - skipping transformation.
INFO:cell_eval._baseline:Filtering out counts from non-targeting
INFO:cell_eval._baseline:Building perturbation-level means
INFO:cell_eval._baseline:Calculating mean of perturbation-level means
INFO:cell_eval._baseline:Assembling baseline adata from perturbation mean
INFO:cell_eval._baseline:Concatenating baseline adata with controls from original adata
INFO:cell_eval._baseline:Saving baseline data to predictions/official_baseline_pred_val.h5ad
... storing 'target_gene' as categorical
✅ cell-eval官方baseline生成成功
Baseline数据形状: (10390, 18080)
扰动条件: target_gene
non-targeting    7483
ARPC2             200
DHX36             200
C1QBP             200
JAZF1             200
HIRA              200
MED1              200
EIF3H             200
TMSB10            200
MED12             200
OXA1L             200
NDUFB6            190
USF2              185
SMARCB1           182
MAX               180
DNMT1             170
Name: count, dtype: int64

✅ 官方baseline生成完成!
保存路径: predictions/official_baseline_pred_val.h5ad

🔍 对比官方baseline与自研baseline...

📊 Baseline对比:
官方baseline: (10390, 18080)
自研baseline: (6115, 18080)

🎯 目标基因分布对比:
目标           官方       自研       差异      
----------------------------------------
ARPC2        200      200      0       
C1QBP        200      200      0       
DHX36        200      200      0       
DNMT1        170      170      0       
EIF3H        200      200      0       
HIRA         200      200      0       
JAZF1        200      200      0       
MAX          180      180      0       
MED1         200      200      0       
MED12        200      200      0       
NDUFB6       190      190      0       
OXA1L        200      200      0       
SMARCB1      182      182      0       
TMSB10       200      200      0       
USF2         185      185      0       
non-targeting 7483     3208     4275    

📈 表达值差异分析:
共同目标数量: 16
  ARPC2: MAE差异 = 0.002639
  C1QBP: MAE差异 = 0.002639
  DHX36: MAE差异 = 0.002639

🎯 下一步:
1. 使用官方baseline重新运行评分
2. 检查DES是否仍然虚高1.0
3. 对比官方baseline与自研baseline的评分差异

📝 运行官方baseline评分:
python stage2_official_celleval_scoring.py
(需要修改输入文件路径为官方baseline)
(torch) sczd425@ln01:~/run/xinby/vcc_local$ python stage2_official_celleval_scoring.py
🎯 阶段2B: cell-eval官方VCC评分
============================================================
✅ 使用官方baseline: predictions/official_baseline_pred_val.h5ad
🔄 准备cell-eval格式数据...
预测数据: (10390, 18080)
真实数据: (6115, 18080)
预测数据扰动条件: ['ARPC2', 'DHX36', 'C1QBP', 'JAZF1', 'HIRA', ..., 'USF2', 'SMARCB1', 'MAX', 'DNMT1', 'non-targeting']
Length: 16
Categories (16, object): ['ARPC2', 'C1QBP', 'DHX36', 'DNMT1', ..., 'SMARCB1', 'TMSB10', 'USF2',
                          'non-targeting']
真实数据扰动条件: ['MED12', 'EIF3H', 'OXA1L', 'HIRA', 'DHX36', ..., 'USF2', 'SMARCB1', 'MAX', 'DNMT1', 'non-targeting']
Length: 16
Categories (16, object): ['ARPC2', 'C1QBP', 'DHX36', 'DNMT1', ..., 'SMARCB1', 'TMSB10', 'USF2',
                          'non-targeting']
✅ 数据格式验证通过

🎯 运行cell-eval官方VCC评分...
WARNING:cell_eval._evaluator:Output directory reports/celleval_official_output already exists, potential overwrite occurring
INFO:cell_eval._evaluator:Input is found to be log-normalized already - skipping transformation.
INFO:cell_eval._evaluator:Input is found to be log-normalized already - skipping transformation.
INFO:cell_eval._evaluator:Computing DE for real data
INFO:pdex._single_cell:Precomputing masks for each target gene
Identifying target masks: 100%|██████████████████████████████| 16/16 [00:00<00:00, 13709.68it/s]
INFO:pdex._single_cell:Precomputing variable indices for each feature
Identifying variable indices: 100%|██████████████████| 18080/18080 [00:00<00:00, 3905294.90it/s]
INFO:pdex._single_cell:Creating shared memory memory matrix for parallel computing
INFO:pdex._single_cell:Creating generator of all combinations: N=289280
INFO:pdex._single_cell:Creating generator of all batches: N=2893
INFO:pdex._single_cell:Initializing parallel processing pool
INFO:pdex._single_cell:Processing batches
Processing batches: 100%|██████████████████████████████████| 2893/2893 [00:09<00:00, 317.33it/s]
INFO:pdex._single_cell:Flattening results
INFO:pdex._single_cell:Closing shared memory pool
INFO:cell_eval._evaluator:Writing real DE results to: vcc_baseline_real_de.csv
INFO:cell_eval._evaluator:Computing DE for pred data
INFO:pdex._single_cell:Precomputing masks for each target gene
Identifying target masks: 100%|██████████████████████████████| 16/16 [00:00<00:00, 10372.31it/s]
INFO:pdex._single_cell:Precomputing variable indices for each feature
Identifying variable indices: 100%|██████████████████| 18080/18080 [00:00<00:00, 3056304.06it/s]
INFO:pdex._single_cell:Creating shared memory memory matrix for parallel computing
INFO:pdex._single_cell:Creating generator of all combinations: N=289280
INFO:pdex._single_cell:Creating generator of all batches: N=2893
INFO:pdex._single_cell:Initializing parallel processing pool
INFO:pdex._single_cell:Processing batches
Processing batches: 100%|██████████████████████████████████| 2893/2893 [00:13<00:00, 209.96it/s]
INFO:pdex._single_cell:Flattening results
INFO:pdex._single_cell:Closing shared memory pool
INFO:cell_eval._evaluator:Writing pred DE results to: vcc_baseline_pred_de.csv
✅ MetricsEvaluator初始化成功
🔄 计算正确的VCC指标: mae, discrimination_score_l1, de_sig_genes_recall...
INFO:cell_eval._pipeline._runner:Computing metric 'de_sig_genes_recall'
INFO:cell_eval._pipeline._runner:Computing metric 'mae'
INFO:cell_eval._types._anndata:Building pseudobulk embeddings for real anndata on: .X
INFO:cell_eval._types._anndata:Building pseudobulk embeddings for predicted anndata on: .X
Iterating over perturbations...: 100%|████████████████████████| 15/15 [00:00<00:00, 1145.78it/s]
INFO:cell_eval._pipeline._runner:Computing metric 'discrimination_score_l1'
Iterating over perturbations...: 100%|███████████████████████| 15/15 [00:00<00:00, 14125.41it/s]
Iterating over perturbations...: 100%|███████████████████████| 15/15 [00:00<00:00, 19741.00it/s]
✅ cell-eval VCC评分完成

📊 解析cell-eval官方评分结果...
results_df类型: <class 'polars.dataframe.frame.DataFrame'>
agg_results_df类型: <class 'polars.dataframe.frame.DataFrame'>
聚合结果列名: ['statistic', 'de_sig_genes_recall', 'mae', 'discrimination_score_l1']
聚合结果形状: (9, 4)
转换为pandas后的内容:
    statistic  de_sig_genes_recall        mae  discrimination_score_l1
0       count            15.000000  15.000000                15.000000
1  null_count             0.000000   0.000000                 0.000000
2        mean             0.814566   0.021610                 0.533333
3         std             0.039334   0.010965                 0.298142
4         min             0.746032   0.013834                 0.066667
5         25%             0.797619   0.014389                 0.333333
6         50%             0.821094   0.017133                 0.533333
7         75%             0.838554   0.022500                 0.800000
8         max             0.875000   0.049048                 1.000000
提取的mean行数据: {'de_sig_genes_recall': 0.8145663243956018, 'mae': 0.021609651297330855, 'discrimination_score_l1': 0.5333333333333333}
✅ 成功提取官方评分:
  mae: 0.0216
  pds: 0.5333
  des: 0.8146

💾 保存官方评分结果...
✅ JSON结果: reports/stage2_official_celleval_scores.json
✅ 文本报告: reports/stage2_official_celleval_report.txt

🔍 对比官方评分 vs 自研评分...

📊 评分对比:
============================================================
指标                   官方评分         自研评分         差异          
============================================================
MAE                  0.0216       0.0214       0.0002      
PDS                  0.5333       0.5000       0.0333      
============================================================

🎉 阶段2B官方评分完成!
✅ 官方评分结果: reports/stage2_official_celleval_scores.json
✅ cell-eval输出: reports/celleval_official_output

📊 官方VCC评分总结:
  mae: 0.0216
  pds: 0.5333
  des: 0.8146

🎯 重要发现:
1. VCC profile中的overlap_at_N指标在registry中不存在
2. 正确的VCC指标是: mae, discrimination_score_l1, de_sig_genes_recall
3. 本脚本使用了正确的官方指标实现