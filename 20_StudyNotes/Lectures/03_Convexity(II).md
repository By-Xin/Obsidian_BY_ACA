---
title: "Convexity (II): Convex Optimization Problems"
tags:
...
> [!info] Reference
> - Boyd & Vandenberghe, Convex Optimization, Section 4.2
...
- 若优化问题有解, 则记所有最优解组成的集合为 $X_{\text{opt}}$:
  ...
  $$
  X_{\text{opt}} = \{x^* \in \operatorname*{arg\,max} f(x) \mid g_i(x) \leq 0, \forall i~; Ax=b\}
  $$
...
> 给定 $\boldsymbol{y} \in \mathbb{R}^n$, $\boldsymbol{X} \in \mathbb{R}^{n \times p}$, LASSO 问题定义为:
>
> $$
> \begin{aligned}
...
> [!example] Example (SVM)
> 考虑如下支持向量机(SVM)的优化问题. 对于给定的训练数据集 $\{(\boldsymbol{x}_i, y_i)\}_{i=1}^n$, 其中 $\boldsymbol{x}_i \in \mathbb{R}^p$ 是样本特征, $y_i \in \{-1, 1\}$ 是样本标签, SVM 的优化问题定义为 (其中 $\boldsymbol{\xi} = (\xi_1, \ldots, \xi_n)^\top$ 是松弛变量, 允许一定程度的分类错误):
>
> $$
> \begin{aligned}
...
> [!example] Example (Minimizing Euclidean Norm)
> 无约束的 Euclidean 范数最小化问题:
>
> $$
> \begin{aligned}
> \min_x \|Ax - b\|_2
...
> [!info] Reference
> - Boyd & Vandenberghe, Convex Optimization, Section 3.2.5, Section 4.1.3
