...
> [!note] Definition (近端算子)
> 对于一个凸函数 $h: \mathbb{R}^n \to \mathbb{R}$, 其近端算子 $\text{prox}_{h}: \mathbb{R}^n \to \mathbb{R}^n$ 定义为:
>
> $$
...
> [!note] Definition (缩放的近端算子)
> 对于一个凸函数 $h: \mathbb{R}^n \to \mathbb{R}$ 以及一个正标量 $t>0$, 其缩放的近端算子 $\text{prox}_{t h}: \mathbb{R}^n \to \mathbb{R}^n$ 定义为:
>
> $$
...
> [!example] Example ($\ell_1$ 的 prox 算子)
> 对于 $h(x) = \|x\|_1$ 其中 $x\in\mathbb{R}^n$ 及 $t>0$, 其 prox 算子 $u = \text{prox}_{t h}(x)$ 的计算结果为:
>
> $$
...
- 其中第三个等式中
  ...
  - Proximal Gradient Mapping 与次梯度的关系为:
    ...
    $$
    \boxed{G_{t_k}(x^{(k)}) - \nabla \phi(x^{(k)}) \in \partial h(x^{(k+1)}) = \partial h(x^{(k)} - t_k G_{t_k}(x^{(k)}))} \quad (\dagger)
    $$
...
> [!abstract] Theorem (Proximal Gradient Descent 的收敛率)
> 在满足上述条件, 并给定步长 $t_k = t \in (0, 1/L]$ 的情况下, 迭代序列 $\{x^{(k)}\}$ 满足
>
> $$
