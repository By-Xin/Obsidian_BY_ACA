...
\footnotesize
  ...
  \begin{itemize}
    ...
    \item Factors are \emph{known, observable (at the end of the trade-day), and shared by all assets}; factor loadings $\beta_{ip}$ and idiosyncratic errors $\varepsilon_{it}$ are asset-specific:
      ...
      \end{itemize}
  \framebreak
    By denoting $\operatorname{Cov}(\mathbf{F}) := \SigmaMat_f \in \RR^{P\times P}$, $\operatorname{Cov}(\boldsymbol{\varepsilon}) := \boldsymbol{\Psi} \in \RR^{N\times N}$ (often assumed to be diagonal, i.e., independent), and the asset covariance as $\SigmaMat \in \RR^{N\times N}$, and taking the covariance of the return model $\mathbf{R} = \one \boldsymbol{\alpha}^\top + \mathbf{F}\mathbf{B}^\top + \boldsymbol{\varepsilon}$ implies
    \[
      \SigmaMat = \mathbf{B} \SigmaMat_f \mathbf{B}^\top + \boldsymbol{\Psi}.
    \]
    \begin{block}{\footnotesize Remark}
          \begin{itemize}
            \footnotesize
...
\begin{frame}{Notation reference}
  \centering\small
  \begin{tabular}{>{\raggedright\arraybackslash}p{.3\linewidth}p{.55\linewidth}}
    ...
    $\SigmaMat_\Param^{(j)} \in \RR^{\Nassets\times \Nassets}$ & model-implied asset covariance matrix ($\SigmaMat$ if not estimated by factor model) \\
...
\begin{frame}{LinReg: Decoupled baseline (predict, then optimize)}
  ...
  \begin{algorithm}[H]
    ...
    Solve the cardinality-constrained problem with by Gurobi to obtain $\mathbf{w}^{(j)}:=\argmin_{w\in\Wset_\Kcard} \mathbf{w}^\top \SigmaMat_{\hat{\Param}}^{(j)} \mathbf{w}$.\;
...
\begin{frame}{End-to-End architecture as a computational graph}
  ...
  \begin{tikzpicture}[
    ...
    \draw[flow] (sample.south) -- ++(0,-4.55)
      -| node[pos=.56, below=7pt, lab] {realized returns $\mathbf R_j$} (loss.south);
    \draw[flow] (sample.north) -- ++(0,.8)
      -| node[pos=.72, above=10pt, lab] {estimate $\widehat{\mathbf\Sigma}_{f,j}$} (cov.north);
  \end{tikzpicture}
  }
\end{frame}
...
\begin{frame}{E2E SDP relaxation layer}
  ...
  % \begin{block}{\small Intuition of the objective}
  %   \footnotesize
  %   \begin{itemize}
  %     \item  The original objective is $\mathbf{w}^\top \SigmaMat_\Param \mathbf{w} = \left\langle \SigmaMat_\Param, \mathbf{W} \right\rangle$ since $\mathbf{W} = \mathbf{w}\mathbf{w}^\top$.
  %     \item $\mathbf{U}$ is a lifted variable that captures the products of decision variables, and the PSD constraint $\mathbf{U} \succeq 0$ ensures that these products are consistent with the original variables.
  %   \end{itemize}
  % \end{block}
\end{frame}
...
\begin{frame}{E2E SDP relaxation layer}
  ...
  \begin{block}{\small Intuition of the constraints w.r.t. $\mathbf{Z}^c$ }
    \footnotesize
    \begin{itemize}
...
\begin{frame}{E2E SDP relaxation layer}
  ...
  \begin{block}{\small Intuition of the constraints w.r.t. $\mathbf{Q}$ }
    \footnotesize
    \begin{itemize}
      ...
      \item In the relaxed case, the PSD condition will gives $({\mathbf{W}_ii} - w_i^2) {\tilde z^c_i} (1-\tilde z^c_i) \geq w_i^2 (\tilde z_i^c)^2$ for all $i$. 
      \begin{itemize}
        \footnotesize
...
\begin{frame}{DPP compliance}
  \footnotesize
  \texttt{CvxPyLayers} is a library for differentiating through convex optimization problems. It requires the problem to comply with DPP (Disciplined Parametrized Programming). As long as the problem is DPP-compliant, the library can automatically solve the optimization problem in forward passes and compute gradients in backward passes.
  \begin{itemize}
...
\small
  ...
  \begin{block}{\footnotesize Intuition of differentiating through optimization}
    ...
    \begin{itemize}
      \item It means that $\Param$ defines a optimization problem, and the solver returns the optimal solution $\mathbf{w}^\star$. 
...
\begin{frame}{Training behavior}
  ...
  \begin{enumerate}
    ...
    \begin{center}
      \includegraphics[width=\textwidth,height=.35\textheight,keepaspectratio]{fig3.png}
    \end{center}


  \item Training cost is not neglectable.
