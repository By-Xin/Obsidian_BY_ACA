(torch) sczd425@ln01:~/run/xinby/vcc_local$ python step2_train_val_split_fixed.py 
🚀 Step 2 修复版: 构建训练/验证数据拆分（按基因拆分，无数据泄漏）
随机种子: 2025
📂 加载数据: data_raw/k562_standardized.h5ad
数据形状: (18465, 18080)
🧬 加载官方基因顺序...
✅ 成功加载18080个基因
前10个基因: ['SAMD11', 'NOC2L', 'KLHL17', 'PLEKHN1', 'PERM1', 'HES4', 'ISG15', 'AGRN', 'RNF223', 'C1orf159']

🔍 完整验证基因顺序一致性...
✅ 基因顺序完全一致
🔍 分析扰动分布（按基因拆分模式）...
总扰动类型数: 54
总细胞数: 18465
控制细胞数: 10691
扰动类型数: 53

可用于验证的扰动 (>=150细胞):
数量: 21
前10个扰动:
target_gene
MED12     573
EIF3H     369
OXA1L     344
HIRA      341
DHX36     269
MED1      254
TMSB10    228
ARPC2     227
C1QBP     210
JAZF1     209

✅ 选定验证扰动: 15个
  1. MED12: 573细胞
  2. EIF3H: 369细胞
  3. OXA1L: 344细胞
  4. HIRA: 341细胞
  5. DHX36: 269细胞
  6. MED1: 254细胞
  7. TMSB10: 228细胞
  8. ARPC2: 227细胞
  9. C1QBP: 210细胞
  10. JAZF1: 209细胞
  11. NDUFB6: 190细胞
  12. USF2: 185细胞
  13. SMARCB1: 182细胞
  14. MAX: 180细胞
  15. DNMT1: 170细胞

训练扰动: 38个
验证扰动: 15个
控制细胞: 训练和验证集都包含

🔄 按扰动基因进行拆分（无泄漏模式）...
控制细胞拆分: 7483训练 + 3208验证

处理验证扰动（每个扰动最多200个细胞）:
  MED12: 573 -> 200细胞
  EIF3H: 369 -> 200细胞
  OXA1L: 344 -> 200细胞
  HIRA: 341 -> 200细胞
  DHX36: 269 -> 200细胞
  MED1: 254 -> 200细胞
  TMSB10: 228 -> 200细胞
  ARPC2: 227 -> 200细胞
  C1QBP: 210 -> 200细胞
  JAZF1: 209 -> 200细胞
  NDUFB6: 190细胞（无需限制）
  USF2: 185细胞（无需限制）
  SMARCB1: 182细胞（无需限制）
  MAX: 180细胞（无需限制）
  DNMT1: 170细胞（无需限制）

📊 拆分统计:
训练细胞数: 11326
  - 扰动细胞: 3843
  - 控制细胞: 7483
  - 扰动类型: 38 (不含控制)
验证细胞数: 6115
  - 扰动细胞: 2907
  - 控制细胞: 3208
  - 扰动类型: 15 (不含控制)

🔒 数据泄漏检查:
✅ 无数据泄漏，验证扰动在训练集中完全不存在

📁 创建拆分数据集...
训练集: (11326, 18080)
训练集扰动分布 (前10个):
target_gene
non-targeting    7483
SDC1              170
HMGCR             166
NDUFB4            166
UQCRQ             166
DPH2              164
WAC               162
ATP6V0B           147
NISCH             144
METTL3            138
Name: count, dtype: int64

验证集: (6115, 18080)
验证集扰动分布:
target_gene
non-targeting    3208
ARPC2             200
DHX36             200
C1QBP             200
JAZF1             200
HIRA              200
MED1              200
EIF3H             200
TMSB10            200
MED12             200
OXA1L             200
NDUFB6            190
USF2              185
SMARCB1           182
MAX               180
DNMT1             170
Name: count, dtype: int64

✅ 验证基因拆分结果...
==================================================
✓ 基因数量一致: 训练:18080, 验证:18080
✓ 基因顺序一致: 一致
✓ 数据类型: float32, float32
✓ 验证扰动匹配: 期望15, 实际15
✓ 无扰动基因泄漏: 无泄漏
✓ 训练集控制细胞: 7483个
✓ 验证集控制细胞: 3208个
✓ 细胞索引无重叠: 无重叠

总体结果: ✅ 通过

💾 保存修复后的拆分数据...
✅ 训练集: splits/train_fixed.h5ad
✅ 验证集: splits/val_fixed.h5ad

💾 保存拆分信息...
✅ 验证基因列表: splits/validation_genes_fixed.csv
✅ 拆分统计: splits/split_statistics_fixed.csv
✅ 详细报告: reports/step2_split_report_fixed.txt

🎉 Step 2 修复完成！
🔒 关键修复: 验证扰动在训练集中完全不存在，无数据泄漏
可以进行下一步：生成验证真值

Step 2 Train-Val Split Report (FIXED - No Data Leakage)
============================================================
Random seed: 2025
Split date: 2025-09-12 21:53:34.109314
CRITICAL FIX: No perturbation gene leakage between train/val

Training Set:
  Cells: 11326
  Genes: 18080
  Perturbations: 38 (不含控制)
  Control cells: 7483
  Training perturbations: ['ATP6V0B', 'ATP6V0C', 'CHMP3', 'COX6C', 'DNAJA3', 'DPH2', 'EIF4B', 'EWSR1', 'FDPS', 'HDAC3', 'HMGCR', 'HTATSF1', 'KIF20A', 'LRPPRC', 'MAT2A', 'MAU2', 'MBTPS1', 'METTL14', 'METTL17', 'METTL3', 'NDUFB4', 'NISCH', 'PPP2R3C', 'RNF20', 'RRM1', 'SDC1', 'SEC62', 'SLC25A3', 'SMAGP', 'SMARCE1', 'SSBP1', 'STRAP', 'SUPT4H1', 'SUPV3L1', 'TFAM', 'UQCRB', 'UQCRQ', 'WAC']

Validation Set:
  Cells: 6115
  Genes: 18080
  Perturbations: 15 (不含控制)
  Control cells: 3208

Validation Perturbations (完全不在训练集中):
  ARPC2: 200 cells
  DHX36: 200 cells
  C1QBP: 200 cells
  JAZF1: 200 cells
  HIRA: 200 cells
  MED1: 200 cells
  EIF3H: 200 cells
  TMSB10: 200 cells
  MED12: 200 cells
  OXA1L: 200 cells
  NDUFB6: 190 cells
  USF2: 185 cells
  SMARCB1: 182 cells
  MAX: 180 cells
  DNMT1: 170 cells

🔒 Data Leakage Check: PASSED
- 验证扰动在训练集中完全不存在
- 仅控制细胞(non-targeting)在两个集合中共存
